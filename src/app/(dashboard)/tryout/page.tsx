    ...
    (async () => {
      try {
        ...
        for (const s of sorted) {
          if (!byTryout.has(s.tryoutId))
            byTryout.set(s.tryoutId, { score: s.score! });
        }
        const mapped = (list ?? [])
          ...
          .map((t) => {
            ...
            if (locked) {
              if (t.isUltimate && !t.isPremium) {
                ...
                lockReason =
                  "Only available for Premium or higher subscription";
              } else {
                lockReason =
                  "Only available for Premium or higher subscription";
              }
            ...
            return {
              ...
              status: (byTryout.has(t.id) ? "completed" : "not-started") as
                | "completed"
                | "not-started",
        ...
        if (!cancelled)
          setError(e instanceof Error ? e.message : "Failed to load tryouts");
      } finally {
      ...
      <div className="space-y-6">
        ...
        {isAuthError && (
          <p className="text-sm text-slate-600">
            Jika kamu pakai Supabase: pastikan{" "}
            <code className="bg-slate-100 px-1 rounded">
              SUPABASE_JWT_SECRET
            </code>{" "}
            di backend <code className="bg-slate-100 px-1 rounded">.env</code>{" "}
            sama dengan JWT Secret dari project Supabase (Project Settings → API
            → JWT Secret). Lalu coba{" "}
            <button
              type="button"
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-600 mt-1">
          Choose a mock test to begin your IUP International Class preparation.
        </p>
      </div>
            ...
            >
              {tryout.isLocked && (
                <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-white/40 backdrop-blur-[3px]">
                  ...
                  <p className="text-xs text-slate-700 font-medium px-4 text-center mt-1">
                    {tryout.lockReason}
                  </p>
                  <button
              ...
              <div
                className={`flex flex-col gap-4 h-full transition-all ${tryout.isLocked ? "blur-[5px] select-none opacity-60 pointer-events-none" : ""}`}
              >
                ...
                <div className="flex items-center gap-4 text-xs text-slate-500">
                  {tryout.totalQuestions != null && (
                    <span>{tryout.totalQuestions} questions</span>
                  )}
